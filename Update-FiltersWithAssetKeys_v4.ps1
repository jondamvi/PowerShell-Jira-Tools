<#
.SYNOPSIS
    Rewrites Jira Cloud filter JQLs that reference migrated asset object keys. v4.

    v4 adds PER-FILTER literal replacement rules for JQL parts that are unique
    to one filter (unsortable ORDER BY terms, dead filter references, etc.):
      -FilterRules @{
          '15483' = @{ ', cf[10553]'                  = '' }
          '1643'  = @{ 'ORDER BY cf[10483]'            = '' }
          '3788'  = @{ 'OR filter = "IT BUS Tickets"' = '' }
      }
    or the same structure as JSON via -FilterRulesFile rules.json:
      { "15483": { ", cf[10553]": "" }, "3788": { "OR filter = \"IT BUS Tickets\"": "" } }
    Rules are LITERAL (not regex), case-sensitive, applied only to their filter
    id, only outside ~ text-search operands, and BEFORE the global
    -FieldRenameMap and the asset-key rewrite. Empty string = remove.
    A rule that matches nothing is reported as WARN (the PUT would fail again);
    rule ids that were never processed are listed at the end (typo guard).

    v3 adds -FieldRenameMap: renames custom field references (renamed/localized
    fields after migration) in the same PUT as the asset-key rewrite, because
    Jira validates the WHOLE JQL on save — a filter with any dead field name
    cannot take the asset-key fix until that name is corrected too.
      -FieldRenameMap @{ 'Anfrageteilnehmer' = 'Request Participants'; 'Rang' = 'Rank'; 'cf[10677]' = 'Rank' }
    Renames apply to quoted ("Name", 'Name') and bare occurrences, and to
    cf[NNNNN] references, everywhere outside ~ text-search operands (incl.
    ORDER BY). A mapping that matches nothing in a filter is a silent no-op,
    so one global map serves all filters. Applied renames are logged per
    filter as INFO lines in the Errors column.
...
param (
    ...
    # Per-filter literal replacement rules: @{ '<filterId>' = @{ '<old>' = '<new>' } }
    [hashtable]$FilterRules,

    # Same structure as JSON file: { "<filterId>": { "<old>": "<new>" } }
    [string]$FilterRulesFile,

    # Custom field rename map: old reference -> new field name. Applied to every
    # processed filter; mappings that don't occur in a filter do nothing there.
    # Keys may be plain names ('Rang'), localized names, or cf[NNNNN] references.
    # Values with spaces/special characters are emitted double-quoted.
    [hashtable]$FieldRenameMap,
    ...
    # Dry-run pre-validation: run each PROPOSED JQL through the search API
    # (read-only, 1 result max). Jira applies the same validation as the filter
    # PUT — unknown fields/values, unsortable ORDER BY terms — so failures show
    # up in the dry run as 'DryRun-WouldFail' instead of at commit time.
    [switch]$ValidateJql,
...
# --- Per-filter rules: merge -FilterRules and -FilterRulesFile into one map ---
$script:RulesMap     = @{}
$script:RulesApplied = @{}
if ($FilterRules) {
    foreach ($fid in $FilterRules.Keys) {
        $script:RulesMap["$fid".Trim()] = [hashtable]$FilterRules[$fid]
    }
}
if ($FilterRulesFile) {
    if (-not (Test-Path $FilterRulesFile)) { throw "-FilterRulesFile not found: $FilterRulesFile" }
    $json = Get-Content -Path $FilterRulesFile -Raw -Encoding UTF8 | ConvertFrom-Json
    foreach ($p in $json.PSObject.Properties) {
        $rules = @{}
        foreach ($rp in $p.Value.PSObject.Properties) { $rules[$rp.Name] = "$($rp.Value)" }
        if ($script:RulesMap.ContainsKey("$($p.Name)")) {
            foreach ($k in $rules.Keys) { $script:RulesMap["$($p.Name)"][$k] = $rules[$k] }
        } else { $script:RulesMap["$($p.Name)"] = $rules }
    }
}

# Apply the literal rules for one filter id to a text-masked JQL. Ordinal,
# case-sensitive, counts occurrences; a non-matching rule is a WARN because the
# filter would fail validation again on save.
function Invoke-FilterRules {
    param([string]$MaskedJql, [string]$FilterId, [System.Collections.Generic.List[string]]$Log)
    $out   = $MaskedJql
    $rules = $script:RulesMap["$FilterId"]
    if (-not $rules) { return $out }
    $script:RulesApplied["$FilterId"] = $true
    foreach ($old in $rules.Keys) {
        $new   = "$($rules[$old])"
        $count = ([regex]::Matches($out, [regex]::Escape("$old"))).Count
        if ($count -gt 0) {
            $out = $out.Replace("$old", $new)
            $Log.Add("INFO: per-filter rule applied: '$old' -> '$new' ($count occurrence(s))")
        }
        else {
            $Log.Add("WARN: per-filter rule matched nothing: '$old' — check exact text (rules are literal, case-sensitive)")
        }
    }
    return $out
}
...
# Apply -FieldRenameMap to a text-masked JQL. Handles "quoted", 'quoted' and
# bare/cf[NNNNN] occurrences; emits the new name double-quoted when it contains
# anything beyond word characters. Logs each applied rename.
function Invoke-FieldRenames {
    param([string]$MaskedJql, [System.Collections.Generic.List[string]]$RenameLog)
    $out = $MaskedJql
    # Longest keys first so overlapping names cannot partially shadow each other
    foreach ($old in ($FieldRenameMap.Keys | Sort-Object { "$_".Length } -Descending)) {
        $new       = "$($FieldRenameMap[$old])"
        $escOld    = [regex]::Escape("$old")
        $newQuoted = '"' + $new + '"'
        $newBare   = if ($new -match '^[A-Za-z0-9_]+$') { $new } else { $newQuoted }
        $count     = 0

        # Quoted occurrences -> normalized to double-quoted new name
        $rQuoted = [regex]"(?i)""$escOld""|'$escOld'"
        $n = $rQuoted.Matches($out).Count
        if ($n) { $out = $rQuoted.Replace($out, $newQuoted); $count += $n }

        # Bare occurrences. Word-ish names get boundary guards (hyphen included,
        # so e.g. a mapping 'Key' can never match inside Legacy-Key); cf[...] and
        # other non-word references match literally outside quotes.
        if ("$old" -match '^\w+$') {
            $rBare = [regex]"(?i)(?<![-\w""'])$escOld(?![-\w""'])"
        } else {
            $rBare = [regex]"(?i)(?<![""'])$escOld(?![""'])"
        }
        $n = $rBare.Matches($out).Count
        if ($n) { $out = $rBare.Replace($out, $newBare); $count += $n }

        if ($count -gt 0 -and $null -ne $RenameLog) {
            $RenameLog.Add("INFO: field rename applied: '$old' -> $newBare ($count occurrence(s))")
        }
    }
    return $out
}
...
function Convert-Jql {
    param([string]$Jql, [System.Collections.Generic.List[string]]$RenameLog, [string]$FilterId)
    $mask = Get-TextMaskedJql -Jql $Jql
    $out  = $mask.Masked
    # Order: per-filter literal rules -> global field renames -> asset-key rewrite.
    # Text operands are masked throughout; the rewrite's generated quoted
    # arguments come last, so nothing earlier can ever touch them.
    if ($FilterId -and $script:RulesMap.ContainsKey("$FilterId")) {
        $out = Invoke-FilterRules -MaskedJql $out -FilterId $FilterId -Log $RenameLog
    }
    if ($FieldRenameMap -and $FieldRenameMap.Count -gt 0) {
        $out = Invoke-FieldRenames -MaskedJql $out -RenameLog $RenameLog
    }
...
# --- Validate a JQL string via the search API (read-only). Returns $null if valid,
#     otherwise Jira's error text (same messages the filter PUT would produce). ---
function Test-JqlValid {
    param([string]$Jql)
    $uri = "$JiraBaseUrl/rest/api/3/search/jql?maxResults=1&fields=id&jql=" + [uri]::EscapeDataString($Jql)
    try   { Invoke-RestMethod -Uri $uri -Headers $script:headers -Method Get | Out-Null; return $null }
    catch { return (Get-JiraErrorDetail $_) }
}
...
function Invoke-FilterProcessing {
    ...
    if (-not $tokenRegex.IsMatch($filter.jql)) {
        # No asset keys — but a filter can still need a field rename to become valid.
        $renameApplicable = $script:RulesMap.ContainsKey("$($filter.id)")
        if ($FieldRenameMap -and -not $renameApplicable) {
            foreach ($old in $FieldRenameMap.Keys) {
                if ($filter.jql -imatch [regex]::Escape("$old")) { $renameApplicable = $true; break }
            }
        }
        if (-not $renameApplicable) {
            if ($Filters -or $FilterIds) { Write-Host "[$($filter.name)] no asset key references found" -ForegroundColor DarkGray }
            return
        }
        Write-Host "[$($filter.name)] no asset keys, but field rename map applies — processing" -ForegroundColor DarkGray
    }
    ...
    if ($alreadyMigrated) {
        ...
        $newJql  = Convert-Jql -Jql $originalJql -RenameLog $errors -FilterId "$($filter.id)"
        ...
        elseif ($ValidateJql -and $changed -and -not $textOnly) {
            $vErr = Test-JqlValid -Jql $newJql
            if ($vErr) {
                $status = 'DryRun-WouldFail'
                $errors.Add("VALIDATION (dry-run): proposed JQL rejected by Jira — $vErr")
            }
            else {
                $status = 'DryRun-Valid'
            }
    ...
    Write-Host "Status       : $status" -ForegroundColor $(
        if ($status -eq 'FAILED' -or $status -eq 'DryRun-WouldFail') { 'Red' }
        elseif ($status -eq 'DryRun-Valid') { 'Green' }
...
if ($script:RulesMap.Count -gt 0) {
    Write-Host "Per-filter rules  : $($script:RulesMap.Count) filter id(s)" -ForegroundColor Cyan
    foreach ($fid in $script:RulesMap.Keys) {
        foreach ($old in $script:RulesMap[$fid].Keys) {
            Write-Host ("  [{0}] '{1}' -> '{2}'" -f $fid, $old, $script:RulesMap[$fid][$old]) -ForegroundColor Cyan
        }
    }
}
if ($FieldRenameMap -and $FieldRenameMap.Count -gt 0) {
    Write-Host "Field rename map  : $($FieldRenameMap.Count) mapping(s)" -ForegroundColor Cyan
    foreach ($old in $FieldRenameMap.Keys) { Write-Host ("  '{0}' -> '{1}'" -f $old, $FieldRenameMap[$old]) -ForegroundColor Cyan }
}
...
if ($Commit) {
    ...
    Write-Host "MODE: DRY RUN — no writes will be made.$(if ($ValidateJql) { ' Proposed JQL will be validated via the search API.' })" -ForegroundColor Green
}
...
if ($ValidateJql -and -not $Commit) {
    $wf = @($script:results | Where-Object { $_.Status -eq 'DryRun-WouldFail' }).Count
    $ok = @($script:results | Where-Object { $_.Status -eq 'DryRun-Valid' }).Count
    Write-Host "Would fail     : $wf" -ForegroundColor $(if ($wf) { 'Red' } else { 'Cyan' })
    Write-Host "Would succeed  : $ok" -ForegroundColor Green
}
$unusedRuleIds = @($script:RulesMap.Keys | Where-Object { -not $script:RulesApplied.ContainsKey($_) })
if ($unusedRuleIds.Count -gt 0) {
    Write-Host "WARNING: per-filter rules defined for filter id(s) never processed in this run: $($unusedRuleIds -join ', ')" -ForegroundColor Red
}
