        ...
        for (Map.Entry<String, List<String>> t : byTarget.entrySet()) {
            ...
            if (v == null) {
                for (String key : t.getValue()) {
                    ...
                    if (d == null || d.trim().isEmpty()) d = mig.discoveredDefaults.get(key)
            ...
            if (v != null) {
                if (mig.valuesNumeric) {
                    BigDecimal num
                    try { num = new BigDecimal(v.trim().replace(',', '.')) }
                    catch (NumberFormatException nfe) {
                        throw new IllegalStateException('value "' + v + '" for "' + t.getKey() + '" is not numeric')
                    }
