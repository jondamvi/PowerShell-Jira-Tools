 ...
 * have caught.
 */
@Field List<String> VERSION_MAP_OVERRIDE = null   // filled from versionMapOverride() at start

/** Paste big affected-version mappings INSIDE this method (own bytecode budget). */
List<String> versionMapOverride() {
    return []
}
...
@Field List<Long> PAGE_IDS_OVERRIDE = null        // filled from pageIdsOverride() at start

/** Paste big page-id lists INSIDE this method (own bytecode budget). */
List<Long> pageIdsOverride() {
    return []
}
 ...
 */
/*
 * PASTE TARGET MOVED - READ THIS.
 * Config literals now live INSIDE METHODS, not in @Field initializers.
 * Reason: every @Field initializer compiles into the script's generated
 * run() method, which the JVM caps at 64 KB - the pasted MIGRATIONS block
 * plus the pipeline shared that one budget, which is what kept failing
 * compilation with "Method too large" on unrelated changes. A method body
 * has its own 64 KB, so config in methods never competes with the flow.
 * Paste your MIGRATIONS entries inside migrationsConfig() below; paste big
 * page-id / version-map lists inside pageIdsOverride() / versionMapOverride().
 */
@Field List MIGRATIONS = null              // filled from migrationsConfig() at start

List migrationsConfig() {
    return [
...
/** PageFinding identity from the loaded page - one call, out of the body. */
void initPageFinding(PageFinding pf, long pid, AbstractPage page, String baseUrl) {
    pf.pageId = pid
    ...
    pf.url = baseUrl + '/pages/viewpage.action?pageId=' + pid
}
...
PAGE_IDS_OVERRIDE = pageIdsOverride()
VERSION_MAP_OVERRIDE = versionMapOverride()
...
try {
    ...
    for (Long pid : scope) {
        ...
        initPageFinding(pf, pid.longValue(), page, baseUrl)
