...
try {
    ...
    for (String m : MACRO_NAMES) {
        needles.add('%ac:name="' + m.replace('%', '') + '"%')
        needleOfMacro.put(m, 'ac:name="' + m + '"')
    }
    ...
    String currentQuery = 'SELECT c.title, c.content_status, s.spacekey, b.body FROM content c ' +
        'LEFT JOIN spaces s ON s.spaceid = c.spaceid LEFT JOIN bodycontent b ON b.contentid = c.contentid ' +
