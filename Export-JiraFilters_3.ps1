...
# Use -w to capture the final URL after redirects
...
$searchArgs = @(
    ...
    "-d", $searchData
    "-H", "Content-Type: application/x-www-form-urlencoded"
    "-w", "`n---FINAL_URL---%{url_effective}---FINAL_URL---"  # Output final URL
    $searchUrl
)

Write-Status "Submitting search form to: $searchUrl" -Color Gray
$searchOutput = & $curlExe $searchArgs

# Parse the output to separate HTML and final URL
if ($searchOutput -match '---FINAL_URL---(.+?)---FINAL_URL---') {
    $finalUrl = $matches[1].Trim()
    $searchResponse = $searchOutput -replace '---FINAL_URL---.*?---FINAL_URL---', ''
    
    Write-Status "✓ Search form submitted successfully" -Color Green
    Write-Status "  Final URL after redirect: $finalUrl" -Color Cyan
    
    # Parse parameters from the final URL
    if ($finalUrl -match '\?(.+)$') {
        $urlParams = $matches[1]
        Write-Status "  URL parameters: $urlParams" -Color Gray
        
        # Extract base URL and parameters separately
        $finalBaseUrl = $finalUrl -replace '\?.*$', ''
        
        # Parse individual parameters for later use
        $searchParams = @{}
        foreach ($param in $urlParams.Split('&')) {
            if ($param -match '^([^=]+)=(.*)$') {
                $searchParams[$matches[1]] = $matches[2]
            }
        }
        
        # Show important parameters
        if ($searchParams.ContainsKey('searchId')) {
            Write-Status "  ✓ Found searchId: $($searchParams['searchId'])" -Color Green
        }
        if ($searchParams.ContainsKey('sessionId')) {
            Write-Status "  ✓ Found sessionId: $($searchParams['sessionId'])" -Color Green
        }
    } else {
    ...
    Write-Status "⚠ Could not capture final URL - using default" -Color Yellow
    $searchResponse = $searchOutput
    $finalBaseUrl = $searchUrl
    $urlParams = ""
    $searchParams = @{}
}

if ($searchResponse -match "mf_browse" -or $searchResponse -match "filter") {
    Write-Status "✓ Search results contain filter table" -Color Green
} else {
    Write-Status "⚠ Search results unclear - continuing anyway..." -Color Yellow
    if ($debugMode) {
...
# Now fetch paginated results using the discovered URL pattern
$pageOffset = 0
$pageNumber = 1
$hasMorePages = $true
while ($hasMorePages) {
    ...
    # Build pagination URL using parameters from the search result
    if ($searchParams.Count -gt 0) {
        # Use the discovered URL pattern with preserved parameters
        $pageParams = $urlParams -replace 'pagingOffset=\d+', "pagingOffset=$pageOffset"
        if ($pageParams -notmatch 'pagingOffset=') {
            $pageParams += "&pagingOffset=$pageOffset"
        }
        $pageUrl = "$finalBaseUrl?$pageParams"
    } else {
