...
@Field int SCAN_CHUNK = 2000              // ROWS examined per query (real rows, not id-range)
@Field int TIME_BUDGET_SECONDS = 90       // stop cleanly under the proxy ceiling
@Field long SCAN_OFFSET = 0               // resume point (last contentid seen) from previous run
@Field long REF_OFFSET = 0                // Ref.Id start; set to the previous run's final Ref.Id
...
try {
    ...
    long nextOffset = SCAN_OFFSET

    // Keyset pagination over REAL rows: walk current+draft PAGE rows ordered by
    // contentid, LIMIT SCAN_CHUNK per query, advancing past the last id seen.
    // This visits only rows that exist - empty id ranges cost nothing - so the
    // scan time is proportional to the number of pages, not to the id range.
    // 'kind' distinguishes a current hit (0) from a draft hit (1) so both are
    // collected in one ordered walk.
    String scanQuery =
        'SELECT c.contentid AS cid, c.draftpageid AS pid, ' +
        "CASE WHEN c.content_status = 'draft' THEN 1 ELSE 0 END AS kind " +
        'FROM content c JOIN bodycontent b ON b.contentid = c.contentid ' +
        "WHERE c.contenttype = 'PAGE' AND c.content_status IN ('current','draft') " +
        'AND c.contentid > :afterId AND (' + orClause + ') ' +
        'ORDER BY c.contentid LIMIT ' + SCAN_CHUNK

    DatabaseUtil.withSql(DB_RESOURCE) { Sql sql ->
        long afterId = SCAN_OFFSET
        boolean stopped = false
        while (true) {
            if (System.currentTimeMillis() - runStart >= budgetMs) { nextOffset = afterId; stopped = true; break }
            Map<String, Object> p = new LinkedHashMap<String, Object>(np)
            p.put('afterId', afterId)
            int rowsThisChunk = 0
            long lastId = afterId
            List<List<Long>> batch = new ArrayList<List<Long>>()
            sql.eachRow(scanQuery, p) { r ->
                long cid = ((Number) r['cid']).longValue()
                int kind = ((Number) r['kind']).intValue()
                Long owningPage = null
                if (kind == 1 && r['pid'] != null) {
                    try { owningPage = Long.parseLong(r['pid'] as String) } catch (Exception ignore) {}
                }
                batch.add([cid, (long) kind, owningPage == null ? -1L : owningPage])
                lastId = cid
                rowsThisChunk++
            }
            for (List<Long> row : batch) {
                if (row.get(1) == 0L) currentHits.add(row.get(0))
                else if (row.get(2) != -1L) draftPageHits.add(row.get(2))
            }
            if (rowsThisChunk < SCAN_CHUNK) { complete = true; nextOffset = lastId; break }
            afterId = lastId
        }
    }
    ...
    int ref = REF_OFFSET, tCurrent = 0, tDraft = 0, tPersonal = 0, tDelete = 0
    ...
    DatabaseUtil.withSql(DB_RESOURCE) { Sql sql ->
        for (Long pid : pageIds) {
            ...
            for (String macro : MACRO_NAMES) {
                ...
                table.append('<tr><td>').append(String.format('%04d', ref + 1))
    ...
    if (!complete) {
        page.append('<p style="font-size:110%;color:#a30"><b>SCAN INCOMPLETE</b> - time budget reached. ')
            .append('Set <b>SCAN_OFFSET = ').append(nextOffset).append('</b> and <b>REF_OFFSET = ')
            .append(ref).append('</b>, then run again to continue ')
