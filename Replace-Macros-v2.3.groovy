/*
 * =============================================================================
 *  CONFLUENCE DC MACRO REPLACEMENT ENGINE  v2.2    ScriptRunner Script Console
 *  (v2.2: strict UserMacro -> ScriptRunner parameter handling - strictParams
 *   fails loud on unknown or unresolvable parameters instead of silently
 *   dropping them; optional valuesNumeric + valueRange validation; several
 *   paramMap keys may feed one target parameter as name aliases)
 *  (v2.1: duplicate-title write failures are reported as Failed rows and the
 *   run continues, instead of terminating - same-title-in-space is a data
 *   defect on pages, not an engine fault)
 * =============================================================================
 *
...
class MigrationDef {
    ...
    /*
     * v2.2 strict parameter handling (UM -> SR):
     *  - strictParams: an on-page parameter not in paramMap, or a mapped target
     *    with no resolvable value, is a per-occurrence failure (Skipped with
     *    reason) - never a silent drop, never an assumed default
     *  - valuesNumeric: resolved values must parse as numbers (comma or dot)
     *  - valueRange: optional [min, max] inclusive bound for numeric values
     */
    boolean strictParams = false
    boolean valuesNumeric = false
    List<BigDecimal> valueRange = null
...
MigrationDef toMigration(Map<String, Object> cfg) {
    try {
        ...
        if (src.get('strictParams') != null)  m.strictParams  = (Boolean) src.get('strictParams')
        if (src.get('valuesNumeric') != null) m.valuesNumeric = (Boolean) src.get('valuesNumeric')
        if (src.get('valueRange') != null) {
            List rr = (List) src.get('valueRange')
            m.valueRange = [new BigDecimal(rr.get(0).toString()), new BigDecimal(rr.get(1).toString())]
        }
...
String replaceGenericMacro(MigrationDef mig, MatchedMacro mm, List<String> notes) {
    try {
        ...
        // Mapped parameters, grouped by TARGET so several source keys (current
        // and historical parameter names) can feed one target parameter; the
        // first key with a page value wins, then declared/discovered defaults.
        // strictParams: exhausted resolution FAILS the occurrence (Skipped with
        // reason) instead of silently dropping the parameter.
        Map<String, List<String>> byTarget = new LinkedHashMap<String, List<String>>()
        for (Map.Entry<String, String> e : mig.paramMap.entrySet()) {
            if (!byTarget.containsKey(e.getValue())) byTarget.put(e.getValue(), new ArrayList<String>())
            byTarget.get(e.getValue()).add(e.getKey())
        }
        for (Map.Entry<String, List<String>> t : byTarget.entrySet()) {
            String v = null
            // phase 1: PAGE values only, across all source names in declared
            // order - a default must never shadow a page value carried under a
            // later (aliased/renamed) name
            for (String key : t.getValue()) {
                if (mm.params.containsKey(key)) {
                    String pv = mm.params.get(key)
                    v = (pv == null || pv.trim().isEmpty()) ? '' : pv.trim()
                    break
                }
            }
            // phase 2: only when NO source name exists on the page - declared,
            // then discovered, defaults
            if (v == null) {
                for (String key : t.getValue()) {
                    String d = mig.paramDefaults.get(key)
                    if (d == null || d.trim().isEmpty()) {
                        d = mig.discoveredDefaults.get(key)
                        // a discovered (macro-definition) default is usable on a
                        // numeric migration only if it IS a number - definition
                        // placeholders like "?" never masquerade as values
                        if (d != null && mig.valuesNumeric) {
                            try { new BigDecimal(d.trim().replace(',', '.').replace('%', '').trim()) }
                            catch (NumberFormatException nfe) { d = null }
                        }
                    }
                    if (d != null && !d.trim().isEmpty()) { v = d.trim(); break }
                }
            }
            if (v == null && mig.strictParams) {
                throw new IllegalStateException('no value for target parameter "' + t.getKey() +
                        '" - tried source name(s) ' + t.getValue() + ' on the page and defaults; ' +
                        'if the macro used another parameter name historically, add it to paramMap')
            }
            if (v != null) {
                if (mig.valuesNumeric) {
                    BigDecimal num
                    String vNum = v.trim().replace(',', '.').replace('%', '').trim()
                    try { num = new BigDecimal(vNum) }
                    catch (NumberFormatException nfe) {
                        // DIAGNOSTIC: dump exactly what the transform received
                        StringBuilder diag = new StringBuilder()
                        diag.append(' | received params {')
                        boolean firstP = true
                        for (Map.Entry<String, String> pe : mm.params.entrySet()) {
                            if (!firstP) diag.append(', ')
                            firstP = false
                            diag.append('"').append(pe.getKey()).append('"=[')
                            String pvv = pe.getValue() == null ? '' : pe.getValue()
                            for (int ci = 0; ci < pvv.length(); ci++) {
                                diag.append(ci == 0 ? '' : ',').append((int) pvv.charAt(ci))
                            }
                            diag.append(']')
                        }
                        diag.append('} macro-id=').append(mm.macroId)
                        throw new IllegalStateException('value "' + v + '" (cleaned "' + vNum +
                                '") for "' + t.getKey() + '" is not numeric' + diag)
                    }
                    if (mig.valueRange != null &&
                            (num.compareTo(mig.valueRange.get(0)) < 0 || num.compareTo(mig.valueRange.get(1)) > 0)) {
                        throw new IllegalStateException('value ' + num.toPlainString() + ' for "' + t.getKey() +
                                '" is outside allowed range [' + mig.valueRange.get(0).toPlainString() + ', ' +
                                mig.valueRange.get(1).toPlainString() + ']')
                    }
                }
                out.put(t.getKey(), mig.valuesNumeric
                        ? v.trim().replace(',', '.').replace('%', '').trim() : v)
            }
        }
        for (Map.Entry<String, String> e : mm.params.entrySet()) {
            if (mig.paramMap.containsKey(e.getKey())) continue      // already handled
            if (mig.strictParams) {
                throw new IllegalStateException('page parameter "' + e.getKey() +
                        '" is not in paramMap - unknown (renamed?) parameter; refusing to guess')
            }
            ...
            try {
                ...
                while (true) {
                    try {
                        ...
                        boolean dupe = cn.contains('DuplicateData') ||
                                        msg.contains('already exists with the title')
                        attempt++
                ...
                if (werr != null && lastWriteEx != null && !dupeFail && !isTolerableError(lastWriteEx)) {
                ...
                if (werr != null) {
                    vf.status = ReplacementStatus.Failed
                    String failText = dupeFail ? 'a page with this title already exists in same space'
                                               : werr + (lastWriteEx == null ? '' : tolerableErrorHint(lastWriteEx))
                    vf.message = failText
                    for (MatchedMacro mm : vf.matchedMacros) {
